<template>
    ...
    <AdminLayout>
        <div class="container mx-auto py-8">
            <h1 class="text-2xl font-semibold text-gray-700 dark:text-gray-300 mb-6">Assign Permissions to Role</h1>
            <form @submit.prevent="assignPermissions">
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-gray-300 font-semibold mb-2">Select Role</label>
                    <select v-model="selectedRole"
                        class="form-select mt-1 block w-full text-gray-700 dark:text-gray-300 dark:border-gray-600 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 dark:focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-500"
                        :class="{ 'border-red-500': form.errors.role_id }" :style="selectStyles">
                        ...
                        <div v-for="permission in permissions" :key="permission.id" class="mr-4 mb-2">
                            <label class="inline-flex items-center dark:text-gray-300">
                                <input type="checkbox" :value="permission.id" v-model="selectedPermissions"
                                    class="form-checkbox dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-indigo-500" />
                                <span class="ml-2">{{ permission.name }}</span>
                            </label>
                        </div>
                    </div>
                    <p v-if="form.errors.permissions" class="text-red-500 text-sm mt-1">{{ form.errors.permissions }}
                    </p>
                </div>

                <div class="flex justify-end">
                    <button type="submit" class="btn-primary dark:bg-blue-700 dark:hover:bg-blue-600 dark:text-gray-300"
                        :disabled="form.processing">Assign Permissions</button>
                </div>
            </form>
        </div>
    </AdminLayout>
